<script lang="ts">
	...
	const avatarSeed = $derived(displayName?.trim() || userEmail?.split('@')[0] || 'socialsent');
			...
			<nav aria-label="Primary" class="workspace-dropdown-container pointer-events-auto relative">
				...
				>
					<img
						src={favicon}
						alt=""
						class="h-7 w-7 rounded-full bg-white object-cover shadow-sm ring-1 ring-stone-200"
						aria-hidden="true"
					/>
