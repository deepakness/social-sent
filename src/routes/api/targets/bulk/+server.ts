...
/**
 * One HTTP round trip for multi-platform cards. The posts page used to POST
 * each target id serially (N fetches + full list reload); a 4-platform card
 * now costs a single request. Cancel/reschedule apply set-based UPDATEs;
 * retry still publishes sequentially per target (provider side effects).
 * Single-id routes stay for API compat — semantics here mirror them exactly.
 */
