...
export const linkedinProvider: PlatformProvider = {
	...
	async publish(content, creds, meta, fetchImpl = providerFetch): Promise<PublishResult> {
		...
		const data = (await res.json().catch(() => ({}))) as { id?: string; urn?: string };
		const urn = restId || data.id || data.urn;
		const remotePostId = urn || `${Date.now()}`;
		return { remotePostId, remoteUrl: urn ? (linkedinPostUrl(urn) ?? undefined) : undefined };
	},
