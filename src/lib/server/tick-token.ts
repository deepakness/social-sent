 ...
 * does need one, and the env secret that used to be the only option has two
 * problems for that job: it cannot be read from outside (so the operator has to
 * invent and store one), and it unlocks `/api/internal/publish` as well as the
 * tick.
 *
...
export interface TickTokenStatus {
	configured: boolean;
	prefix: string | null;
	createdAt: Date | null;
}
