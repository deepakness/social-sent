<script lang="ts">
	...
	const avatarFallback = $derived(
		`https://api.dicebear.com/7.x/notionists/svg?seed=${encodeURIComponent(displayName || 'socialsent')}`
	);
	...
	function platformMax(platform: PlatformId, list: Connection[]): number {
		...
		const maxes = list
			.filter((c) => c.platform === 'mastodon')
			.map((c) => c.metaJson?.maxCharacters)
	...
	let maxChars = $derived.by(() => {
		if (activeTab !== 'global') return platformMax(activeTab, connections);
		...
		const limits = selectedAccounts.map((a) => {
			if (a.platform === 'bluesky') return 300;
			if (a.platform === 'mastodon') return platformMax('mastodon', connections);
			if (a.platform === 'linkedin') return 3000;
			if (a.platform === 'threads') return 500;
			if (a.platform === 'x') return 280;
			return 280;
		});
	...
	const counters = $derived.by(() => {
		...
		return {
			bluesky: { len: maxThreadSegmentLength(bskyText, countGraphemes), max: 300 },
			mastodon: {
				len: maxThreadSegmentLength(mastoText, mastodonWeightedLength),
				max: platformMax('mastodon', connections)
			},
			linkedin: { len: countGraphemes(linkedinFlat), max: 3000 },
			threads: { len: maxThreadSegmentLength(threadsText, countGraphemes), max: 500 },
			x: { len: maxThreadSegmentLength(xText, countGraphemes), max: 280 }
		};
	});
	...
	function getPlatformStatus(platform: PlatformId) {
		...
		const max = platformMax(platform, connections);
	...
	function applyRelativeFields() {
		let d = '0',
			h = '0',
			m = '0';
		if (relativeUnit === 'days') d = relativeValue;
		if (relativeUnit === 'hours') h = relativeValue;
		if (relativeUnit === 'mins') m = relativeValue;
		const value = scheduleFromDHM(d, h, m, new Date());
		schedDate = value.slice(0, 10);
		schedTime = value.slice(11, 16);
	}

	const schedulePreview = $derived.by(() => {
		if (!scheduleCombined || !isFutureScheduleValue(scheduleCombined, new Date())) return null;
		const d = new Date(scheduleCombined);
		if (Number.isNaN(d.getTime())) return null;
		return `${formatLocalDateTimeWithZone(d)} (${formatRelativeTime(d)})`;
	});
	...
	function handleSplitPaste(segmentIndex: number, pasted: string): boolean {
		...
		if (activeTab !== 'global') {
			cap = platformMax(activeTab, connections);
...
>
	...
	<div class="sr-only" role="status" aria-live="polite">{publishAnnouncement}</div>
	{#if toastMessage}
